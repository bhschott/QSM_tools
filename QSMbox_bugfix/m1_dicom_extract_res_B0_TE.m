...
if ~isdir( DicomFolder )
    ...
    dicom_filename = filelist(1).name;
    if contains(dicom_filename, '._')
        dicom_filename = dicom_filename(3:end);
    end
    ttt = [ DicomFolder '/' dicom_filename ]
    info = dicominfo( ttt );
    ...
    try
        ...
        TE = info.EchoTime;
        if TE > 1
            TE = TE/1000        %  QSMbox needs TE in seeconds!
        end
    end
